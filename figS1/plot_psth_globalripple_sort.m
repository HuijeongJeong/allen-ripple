...
fHandle = figure('PaperUnits','Centimeters','PaperPosition',[2 2 5 20/3]);

axes('Position',axpt(10,1,[1:7],1));
hold on;
...
set(gca,'Box','off','TickDir','out','FontSize',7,'LineWidth',0.35,...
    ...
    'YTick',400:400:1600);
ylabel('Cotical neuron #');
title('Global');
xlabel('Time from ripple onset(s)');

h(1) = axes('Position',axpt(10,1,8:9,1));
s = score(celltype(:,1),1:3);
imagesc(1:3,1:size(d,1),s(sortidx,:));
set(h(1),'YTickLabel',[],'Fontsize',7,'Box','off','TickDir','out');
colormap(h(1),'gray')
...
h(2) = axes('Position',axpt(10,1,10,1));
s = score(celltype(:,1),1:3);
imagesc(1,1:size(d,1),c(sortidx,:));
colormap(h(2),ct);
set(h(2),'YTickLabel',[],'Fontsize',7,'Box','off','TickDir','out','XTick',[]);
