...
[structure_vis,ccf_hippo,celltype_hippo] = deal(cell(nS,1));
...
for iS = 1:nS
    ...
    [inhippo,idx] = ismember(T.unit_id,tag.info.unit_id(strcmp(tag.info.structure,'CA1')));
    
    unitid_tmp = [T.unit_id(invis); T.unit_id(inhippo)];
    areaidx = false(sum(invis|inhippo),2);
    ...
    [~,idxx] = ismember(unitid_tmp(areaidx(:,2)),tag.info.unit_id);
    celltype_hippo{iS} = [tag.celltype.rs(idxx), tag.celltype.fs(idxx)];
    ...
    ccf_hippo{iS} = [T.anterior_posterior_ccf_coordinate(idxx), T.left_right_ccf_coordinate(idxx),...
        T.dorsal_ventral_ccf_coordinate(idxx)];
    ...
    for iState = 1:3
        ...
        [~,~,ncorr_sp{iS,iState},time_sp(iS,iState)] =...
            noisecorr_nostim([T.spike_time(invis);T.spike_time(inhippo)],...
...
locList = {'';'ccf ML: <7.8mm';'ccf ML: >8.8mm'};
refList2 = {'d';'i'};
locIdx = cellfun(@(x) [true(size(x,1),1), x(:,2)<7800, x(:,2)>=8800],ccf_hippo,'UniformOutput',false);

iCT = 1;
iState = 1;
fHandle = figure('PaperUnits','Centimeters','PaperPosition',[2 2 6 3]);
for iLoc = 1:3
    axes('Position',axpt(3,10,iLoc,2:9,axpt(10,1,2:10,1)));
    hold on;
    nc_sp = NaN(nS,k+1);
    inhippo = cellfun(@(y,z) y(:,iCT) & z(:,iLoc),...
        celltype_hippo,locIdx,'UniformOutput',false);
    for iClst = 1:k+1
        nc_sp(:,iClst) = cellfun(@(x,y,z) nanmean(nanmean(x(y==iClst-1,z),2)),...
            ncorr_sp(:,iState),cidx_vis,inhippo);
    end
...
print(fHandle,'-dtiff','-r600','noise_corr_ca1_woripple.tif');
...
%             structure_hippo(~out),celltype_hippo(~out),locIdx(~out),'UniformOutput',false);
        for iClst = 1:k+1
            nc_dg(:,:,iClst) = cellfun(@(x,y,z) nanmean(nanmean(x(y==iClst-1,z),2)),...
                ncorr_dg(:,:,iRef),repmat(cidx_vis,1,3),repmat(inhippo,1,3));
            nc_sp(:,:,iClst) = cellfun(@(x,y,z) nanmean(nanmean(x(y==iClst-1,z),2)),...
                ncorr_sp(:,:,iRef),repmat(cidx_vis,1,3),repmat(inhippo,1,3));
            sc_dg(:,:,iClst) = cellfun(@(x,y,z) nanmean(nanmean(x(y==iClst-1,z),2)),...
                scorr_dg(:,:,iRef),repmat(cidx_vis,1,3),repmat(inhippo,1,3));
        end
