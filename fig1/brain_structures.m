...
structureList = {'VISp_385','VISl_409','VISal_402','VISrl_417','VISam_394','VISpm_533','LGd_170','LP_218','CA1_382'}';
structurenum = cellfun(@(x) strsplit(x,'_'),structureList,'UniformOutput',false);
structurenum = cellfun(@(x) x{2},structurenum,'UniformOutput',false);
...
hold on;
for iS = 1:length(structureList)
    fileID = fopen([meshDir,structureList{iS},'.txt']);
    if fileID==-1
        websave([meshDir,structureList{iS},'.txt'],[url,'/',structurenum{iS},'.obj']);
        fileID = fopen([meshDir,structureList{iS},'.txt']);
    end
...
data(data(:,2)>5.7,:) = [];
    ...
    trisurf(k,data(:,1),data(:,2),data(:,3),'FaceColor',[0.8 0.8 0.8],...
