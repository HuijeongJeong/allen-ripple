...
fHandle = figure('PaperUnits','Centimeters','PaperPosition',[2 2 20*0.15 20/3]);
for iunit = 1:size(rippleidx,2)
    in = find(cidx==iunit);
    satisfied = 0;
    h(1+(iunit-1)*2) = axes('Position',axpt(1,2,1,1,axpt(1,3,1,iunit,[],[0.025 0.025])));hold on;
    h(2+(iunit-1)*2) = axes('Position',axpt(1,2,1,2,axpt(1,3,1,iunit,[],[0.025 0.025])));hold on;
    if isnan(iList(iunit))
        i = 0;
        while satisfied==0
            cla(h(1+(iunit-1)*2));
            cla(h(iunit*2));
            axis(h([1:2]+(iunit-1)*2),'off');
            i = i+1;
            spikeripple = spikeWin(spiketime{in(i)},rippletime,win);
            [xpt,ypt,time,~,spkconv,~,spksem] = rasterPSTH(spikeripple,rippleidx,win,bin,resolution);
            h(1+(iunit-1)*2) = axes('Position',axpt(1,2,1,1,axpt(1,3,1,iunit,[],[0.025 0.025])));hold on;
            h(2+(iunit-1)*2) = axes('Position',axpt(1,2,1,2,axpt(1,3,1,iunit,[],[0.025 0.025])));hold on;
            for iref = 1:2
                scatter(xpt{iref},ypt{iref},0.5,'.',ct{iref},'Parent',h(1+(iunit-1)*2));
                fill([time flip(time)],spksem(iref,:),ct{iref},'EdgeColor','none','Parent',h(2+(iunit-1)*2));
                plot(time,spkconv(iref,:),'Color',ct{iref},'Parent',h(2+(iunit-1)*2));
            end
            satisfied = input('satisfied?');
        end
    else
        spikeripple = spikeWin(spiketime{in(iList(iunit))},rippletime,win);
        [xpt,ypt,time,~,spkconv,~,spksem] = rasterPSTH(spikeripple,rippleidx,win,bin,resolution);
        for iref = 1:2
            scatter(xpt{iref},ypt{iref},0.5,'.',ct{iref},'Parent',h(1+(iunit-1)*2));
            fill([time flip(time)],spksem(iref,:),ct{iref},'EdgeColor','none','Parent',h(2+(iunit-1)*2));
    ...
    y2 = get(h(2+(iunit-1)*2),'YLim');
    if y2(2)<2
        y2(2) = 2;
    end
    plot([0 0],y2,'k:','Parent',h(2+(iunit-1)*2));
    plot([0 0],[1 find(rippleidx(:,2),1,'last')],'k:','Parent',h(1+(iunit-1)*2))
    set(h([1:2]+(iunit-1)*2),'Box','off','TickDir','out','XLim',[-1.5 1.5],'FontSize',7)
    set(h(1+(iunit-1)*2),'YTick',find(rippleidx(:,2),1,'last'),'YLim',[1 find(rippleidx(:,2),1,'last')])
    set(h(2+(iunit-1)*2),'YTick',[ceil(y2(1)),floor(y2(2))],'YLim',y2)
    alpha(0.2);
    if iunit==3
        xlabel(h(2+(iunit-1)*2),'Time (s)');
    else
        set(h(2+(iunit-1)*2),'XTickLabel',[])
    end
        ylabel(h(1+(iunit-1)*2),'Ripple');
        ylabel(h(2+(iunit-1)*2),'Rate (Hz)')
end
cd('D:\OneDrive - University of California, San Francisco\figures\allen\fig1')
print(fHandle,'-depsc','-painters','psth_ex.ai')